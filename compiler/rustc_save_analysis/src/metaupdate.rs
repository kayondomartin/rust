...
pub struct SpecialTypes {
    ...
    pub field_exprs: FxHashMap<NodeId, FxHashSet<u32>>,
    pub unbox_exprs: FxHashMap<NodeId, FxHashSet<u32>>
}
...
pub struct ExternStructField {
    ...
    used_exprs: FxHashMap<String, FxHashMap<NodeId, FxHashMap<u32, Option<FieldExprAction>>>>
}
        ...
        JsonExternStructField {
            ...
            uses: self.used_exprs.iter().map(|(crate_name, uses_map)|{
                (crate_name.clone(), uses_map.iter().map(|(owner, locals)|{
                    (owner.as_usize(), locals.iter().map(|(i, actions)| {
                        (*i, match actions {
                                Some(action) => {
                                    match action {
    ...
    fn to_extern_struct_field(&self) -> ExternStructField {
        ExternStructField {
            ...
            used_exprs: self.uses.iter().map(|(crate_name, hir_ids)|{
                (crate_name.clone(), hir_ids.iter().map(|(node_id, offsets)|{
                    (NodeId::from_usize(node_id), offsets.iter().map(|(i, j)|{
                        (*i, match j {
            ...
            for (_field_name, field) in fields.iter(){
                ...
                if field.should_box {
                    ...
                    if let Some(field_exprs) = extern_struct_field.used_exprs.get(&String::from(crate_name)) {
                        for (node_id, locals) in field_exprs {
                            ...
                            if let Some(set) = special_types.field_exprs.get_mut(node_id) {
                                set.extend(field_locals.into_iter());
                            }else{
                                special_types.field_exprs.insert(*node_id, field_locals);
                            }

                            /*if let Some(set) = special_types.unbox_exprs.get_mut(node_id) {
                                set.extend(unboxable_locals.into_iter())
                            }else{
                                //special_types.unbox_exprs.insert(*node_id, unboxable_locals);
                            }*/
                        }
